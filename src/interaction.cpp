...
void Game::op_info() {
    ...
    for (int i = 0; i < m_map->m_cell_count; i++) {
        map_cli[(m_map->cells()[i].pos.x - 1 + m_map->m_col / 2) +
                (m_map->cells()[i].pos.y - 1 + m_map->m_row / 2) *
                    m_map->m_col] = m_map->cells()[i].pos.h + 1;
    }

    for (int i = 0; i < m_map->m_light_count; i++) {
        map_cli[(m_map->lights()[i].pos.x - 1 + m_map->m_col / 2) +
                (m_map->lights()[i].pos.y - 1 + m_map->m_row / 2) *
                    m_map->m_col] +=
            (100 + int(m_map->lights()[i].turned_on) * 1000);
    }

    map_cli[(m_bot->current_position().x - 1 + m_map->m_col / 2) +
            (m_bot->current_position().y - 1 + m_map->m_row / 2) *
                m_map->m_col] += 10000;
